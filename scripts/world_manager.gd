...
func save_animal_state(animal):
	...
	# If the animal has no parent segment (e.g. test dummies added to a non-world
	# node like TestRunner), abort early — find_segment_containing_position would
	# otherwise return the nearest world segment purely by proximity and cause
	# test-artifact IDs to leak into animals_state.
	var current_segment = get_segment_for_animal(animal)
	if not current_segment:
		if DebugLogger.animal_state: print("[SAVE ERROR] Animal not in any world segment (skipped):", animal_id)
		return
	# Determine the target segment by global position so that animals that have
	# crossed a segment boundary (local_pos out of [0, world_width)) are saved
	# to the correct neighbour segment regardless of their current parent.
...
func check_and_create_missing_animal(segment: Node2D):
	...
	for animal_id in animals_state:
		...
		# Skip IDs that look like test artifacts or Godot-autogenerated renames.
		# Matches the purge logic in clear_active_animal.
		var _raw_check: String = animal_id.trim_prefix("animal/")
		if _raw_check.begins_with("_") or "@" in _raw_check:
			continue
